...
export class CardInfo extends ChildComponent {
    ...
    fillElements() {
        $R(this.element).html(renderService.htmlToElement(template, [], styles).innerHTML);
        $R(this.element).findAll(':scope > div').forEach(child => child.addClass('fade-in'));
        $R(this.element).find('#card-number').text(formatCardNumber(this.card.number)).click(this.#copyCardNumber.bind(this));
        ...
        $R(this.element).find('#toggle-cvc').click(this.#toggleCvc.bind(this, cardCvcElement));
